...
// Coste O(MlogM) siendo M el número de cartas
bool tratar_caso() {
  int N, M;
  cin >> N >> M;
  if (!cin)
  ...
  while (M--) {
    ...
    tam = juego.at(i).size();
    juego.at(i).insert(value); //logM
    if (juego.at(i).size() == tam)
      juego.at(i).erase(value); //logM
